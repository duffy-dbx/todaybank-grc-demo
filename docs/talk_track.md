# Axos GRC Operations Hub — Talk Track
...
1. Catalog Explorer at `axos_grc_demo`
2. Notebook `01_lakeflow_pipeline.py` (pipeline DAG view)
3. MLflow registry view of `axos_grc_demo.gold.aml_risk_model`
4. AI/BI Dashboard "Axos GRC Operations Hub"
5. Genie space "Axos GRC Operations Hub"
...
> "Here's the day in the life of an Axos Risk Officer that you've probably seen.
>
...
1. **Open Catalog Explorer** at `axos_grc_demo`.
   > "This is everything in our Axos GRC demo — bronze raw landing, silver cleansed, gold business-ready, plus an audit schema. Notice that it's one tree."
...
3. **Click "Sample Data" — show real values.** Then run:
   ```sql
   SELECT * FROM axos_grc_demo.silver.customers_v LIMIT 5;
   ```
...
1. **Open a SQL editor and run:**
   ...
   FROM axos_grc_demo.bronze.raw_complaints
   LIMIT 10;
   ```
...
1. **Open MLflow registry view** of `axos_grc_demo.gold.aml_risk_model`.
...
1. **Open the Axos GRC Operations Hub dashboard.**
...
>
> The good news for Axos is you already have most of this. You're live on the lakehouse. You have AI/BI. You're evaluating Genie. What we'd love to do next is help you stand up exactly this kind of unified GRC view on your real data — and the patterns you saw today are the same patterns we'd use."
...
| "How long would this take to stand up at Axos?" | The demo footprint took about 15 minutes to deploy. A production version with your real source feeds is typically a 6–10 week engagement, parallel to your existing systems. |
