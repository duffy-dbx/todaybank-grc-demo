# Axos GRC Demo — Data Engineer Talk Track (4 min)
...
2. Catalog Explorer at `axos_grc_demo.gold.aml_alerts` (Lineage tab ready to open)
...
> "Two rows dropped on `valid_email`. They're quarantined in a side table, not silently passed downstream and not crashing the job. The same `expect_or_drop` line that you saw in the code produced this. If you want to investigate: `SELECT * FROM axos_grc_demo.system.expectations WHERE status = 'DROPPED'` — it's a real table."

---

### 1:45 — Unity Catalog lineage is automatic, not a side project (1 min)

Switch to Catalog Explorer → `axos_grc_demo.gold.aml_alerts` → Lineage tab.
...
> ```
>
> That's your CI/CD hook. Point your GitHub Actions or GitLab runner at those two commands. The bundle handles catalog name interpolation across environments — your dev pipeline writes to `axos_grc_demo_dev`, prod writes to `axos_grc_demo`. No find-and-replace in YAML files."
